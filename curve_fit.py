...
# PROJECT: Curve Fit
# PURPOSE:
# This script aims to reduce the time spent converting Rainfall-
# Intensity Duration Frequency Curves (RIDF) into usable rainfall
# data for Hydrologic and Hydraulic Simulations
# Function requires the following:
# 1) The path to the location (folder) of the rainfall data
# 2) File name of the rainfall data
# 3) File name and location of the output
# AUTHOR: Ned Santiago, started on November 28, 2022.
# 
# Notes:
# This script uses SciPy, SKLearn, or Tensorflow libraries for
# Curve Fitting
# 
# Future planned updates:
# 1) Selection of rainfall stations
# 2) Use as a plugin to other programs
# 3) GitHub integration
# 4) Optimization
# 5) Provide a GUI to the DataRetriever object
#
#
#####################################################################
...
#folder_path = r"~/Documents/PY/CurveFit"
#file_name = r"RIDFButuanMMHR.csv"
#formula = hoerl_mod
#
## Using the DataRetriever, create the path for the file
#ridf_retriever = DataRetriever()
#file_path = ridf_retriever.full_path(folder_path = folder_path,
#    file_name = file_name)
#
## Using the Ridf object, create a Pandas DataFrame of the RIDF table. Then clean and organize
#ridf = Ridf(ridf_retriever.file_path)
#ridf.time_scale()
#
## Using the CurveFitter object, get the constants to replicate the data's trend
#cf = CurveFitter()
#cf.curvefit(formula, ridf.df)
#
## Create the data to be graphed onto an Intensity-Duration Table
#orig_xdata = np.array(ridf.df.columns)
#orig_ydata = ridf.df.iloc[0,:]
#xdata = np.linspace(min(orig_xdata), max(orig_xdata),100)
#ydata = cf.graph_data(xdata,formula)
#
## Using the Grapher, create the figure with the data provided
#ridf_graph = Grapher()
##ridf_graph.grapher(xdata,ydata,orig_xdata,orig_ydata)
...
# Declaring the folder location, file name, and formula
folder_path = r"~/Documents/PY/CurveFit"
file_name = r"IloiloMMHR2019.csv"
export_path = "~/Documents/PY/CurveFit/IloIloMMHR_rainfall.csv"
formula = hoerl_mod
